...
int main(void) {
  ...
  Value *a_ptr, *b_ptr, *c_ptr, *sum, *dif;
  { // create entryBB
  ...
  { // create falseBB
    ...
    dif = builder.CreateSub(b, c, "dif");
    builder.CreateBr(mergeBB);
  }
  { // create mergeBB
    builder.SetInsertPoint(mergeBB);
    PHINode* d = builder.CreatePHI(Type::getInt32Ty(ctx), 2, "d");
    d->addIncoming(sum, trueBB);
    d->addIncoming(dif, falseBB);
